...
class Yoga_model extends CI_Model{
    ...
    public function updateyoga($post_data)
    {
        $this->db->where('id',$post_data['id']);
        if($this->db->update('yoga',$post_data))
            return TRUE;
        else
            return FALSE;
    }
    ...
    {           
        return $this->db->query("SELECT * FROM astro_yoga")->num_rows();
    }

    public function record_count1()
    {
        $today = date("Y-m-d");      
        return $this->db->query("SELECT * FROM astro_yoga WHERE date_from <= '$today' AND date_to <= '$today'")->num_rows();
    }

    public function getyoga($lim_to,$lim_from,$s_key="")
    {         
        $this->db->limit($lim_to,$lim_from); 
        $rs = $this->db->get_where("yoga");   
